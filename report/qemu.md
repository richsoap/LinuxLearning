...
2. 在第一步中列出的样例列表中找到qemu_x86_64_defconfig，然后再使用
```shell
...
make menuconfig
```
命令，编译出选项菜单的图形界面，在"System configuration->Root password"中配置上我们所需要的密码,若此项为空，则无需密码。
4. 使用
```
make -j8
```
...
命令加载系统，需要用到的选项如下
<ul>
<li>-kernel 内核位置 此处为output/images/bzImage</li>
<li>-drive 驱动器位置 此处即为我们的ext2文件系统，还需要参数format说明文件系统格式，if说明访问媒体的层次</li>
...
uname -a
```
命令查看运行系统的版本号为"4.9.6"，完成基本的Linux系统编译与运行。
截图如下 ![](photo/qemu_version.png)
