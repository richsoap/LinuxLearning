...
void callback(void* para) {
	...
	dma_unmap_single(mypci_dev->dev,mypci_dev->dma_addr,mypci_dev->dma_size,mypci_dev->dma_dir);
	complete(&Comp);
}
...
int dev_open(struct inode *inode,struct file*filp) {
	...
	//for dma
	if(using_count == 0){
		alloc_cpumask_var(mypci_dev->mask,GFP_KERNEL);
		mypci_dev->dma_chan = dma_request_channel(*(mypci_dev->mask),NULL,NULL);
	}
...
int dev_release(struct inode *inode,struct file*file) {
	using_count --;
	if(using_count == 0) {
		dma_release_channel(mypci_dev->dma_chan);
	}
	return 0;
}
...
ssize_t dev_read(struct file*file,char *buf,size_t count,loff_t *ppos) {
	...
	*mypci_dev->addr = dma_map_single(mypci_dev->dev,mypci_dev->buffer,count,DMA_FROM_DEVICE);
	mypci_dev->dma_size = count;
	mypci_dev->dma_dir = DMA_FROM_DEVICE;
	config.direction=DMA_FROM_DEVICE;
	config.dst_addr=*(mypci_dev->addr);
	config.src_addr=0x40000+*ppos;
	config.dst_maxburst=16;
	config.dst_addr_width=8;
	config.src_addr_width=8;
	dmaengine_slave_config(mypci_dev->dma_chan,&config);
	desc = dmaengine_prep_slave_single(mypci_dev->dma_chan,*mypci_dev->addr,count,DMA_FROM_DEVICE,1);
	desc->callback=&callback;
	dmaengine_submit(desc);
	dma_async_issue_pending(mypci_dev->dma_chan);
...
ssize_t dev_write(struct file *file, const char*buf,size_t count,loff_t *ppos) {
	...
	*mypci_dev->addr = dma_map_single(mypci_dev->dev,mypci_dev->buffer,count,DMA_FROM_DEVICE);
	mypci_dev->dma_size = count;
	mypci_dev->dma_dir = DMA_TO_DEVICE;
	config.direction=DMA_TO_DEVICE;
	config.src_addr=*mypci_dev->addr;
	config.dst_addr=0x40000+*ppos;
	config.dst_maxburst=16;
	config.dst_addr_width=8;
	config.src_addr_width=8;
	dmaengine_slave_config(mypci_dev->dma_chan,&config);
