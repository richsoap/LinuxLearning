...
pci_unmap_single(mypci_dev->pdev,mypci_dev->dma_addr,mypci_dev->dma_size,mypci_dev->dma_dir);
	...
	clear_dma_ff(mypci_dev->dma_chan);
	printk(KERN_ERR "clearff\n");
	set_dma_mode(mypci_dev->dma_chan,write?DMA_MODE_WRITE:DMA_MODE_READ);
	set_dma_addr(mypci_dev->dma_chan,bus_addr);
	set_dma_count(mypci_dev->dma_chan,count);
	printk(KERN_ERR "after set\n");
	enable_dma(mypci_dev->dma_chan);
	printk(KERN_ERR "after enable\n");
